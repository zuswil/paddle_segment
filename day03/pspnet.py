...
class PSPModule(Layer):
    ...
    def forward(self, inputs):
        ...
        for idx, f in enumerate(self.features):
            x = fluid.layers.adaptive_pool2d(inputs, self.bin_size_list[idx])  # 一变四
            x = f(x)
            # 64 * 64
            x = fluid.layers.interpolate(x, inputs.shape[2::], align_corners=True)
            out.append(x)
        out = fluid.layers.concat(out, axis=1)  # concat list -- NCHW
...
class PSPNet(Layer):
    def __init__(self, num_classes=59, backbone='resnet50'):
        ...
        # psp: 2048 -> 2048*2
        self.pspmodule = PSPModule(num_channels=num_channels, bin_size_list=[1, 2, 3, 6])
